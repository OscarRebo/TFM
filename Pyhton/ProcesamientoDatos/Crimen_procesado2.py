...
crimen["Fin Semana"]=crimen["Dia Semana"].apply(lambda x: "Si" if(x==4 or x==5 or x==6)else "No")
...
Q1 = crimen.select_dtypes(np.number).quantile(0.25)
Q3 = crimen.select_dtypes(np.number).quantile(0.75)
IQR = Q3 - Q1
print(IQR)
superior=Q3+1.5*IQR
inferior=Q1-1.5*IQR
...
frecuencia=crimen["Domestic"].value_counts()
print(frecuencia)
crimen=crimen_sin_outliers
# crimen=crimen.drop(["Latitude","Longitude"],axis=1)
# del crimen_sin_outliers
crimen.info()
crimen.columns
crimen.to_csv("D:/MASTER/Chicago/crimen2.csv")